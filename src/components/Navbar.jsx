import React from 'react'

const Navbar = () => {
  return (
    <nav className='bg-slate-800 text-white '>
      <div className='myconatiner flex justify-around items-center px-5 py-5 h-16'>
  
        <div className='logo font-bold text-white text-2xl'> 
        <span className='text-green-600'> &lt;</span>Pass<span className='text-green-600'>Draft/&gt;</span>
        </div>
        <div className='hidden md:block'>
        <ul className=''>
            <li className='flex gap-4'>
                <a className='hover:font-bold hover:underline' href="/">HOME</a>
                <a className='hover:font-bold hover:underline' href="#">ABOUT</a>
                <a className='hover:font-bold hover:underline' href="#">CONTACT</a>
            </li>
        </ul>
        </div>
        
        <button className='text-white bg-green-700 my-5 rounded-xl  flex justify-between items-center hover:bg-green-500 hover:text-black outline ring-white ring-1'>
          <img className='invert w-10 p-1' src="data:image/png;base64," alt="" />
          <span className='font-bold px-2'>github</span>
        </button>
        </div>
    </nav>
  )
}

export default Navbar
